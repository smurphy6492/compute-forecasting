...
ax.plot(
    daily_total["date"], daily_total["compute_hours"], alpha=0.2, color="#90CAF9", linewidth=0.5
)
rolling_28 = daily_total.set_index("date")["compute_hours"].rolling(28, center=True).mean()
ax.plot(
    rolling_28.index, rolling_28.values, color="#1565C0", linewidth=2.5, label="28-day rolling avg"
)
...
for i, (_, evt) in enumerate(events.iterrows()):
    ...
    ax.annotate(
        evt["event_name"],
        xy=(evt["event_date"], y_max * offsets[i % len(offsets)]),
        fontsize=7,
        color=color,
        fontweight="bold",
...
for ax, (ctype, seg) in zip(axes.flat, series_to_plot, strict=True):
...
fig.suptitle(
    "Hybrid Model Forecast vs. Actual - Test Set (Jan-Jun 2026)",
    fontsize=14,
    fontweight="bold",
    y=1.01,
)
...
fig.suptitle(
    "Enterprise GPU Training - Extrapolation Problem Solved", fontsize=14, fontweight="bold", y=1.02
)
plt.tight_layout()
